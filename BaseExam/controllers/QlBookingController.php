...
public function paySubmit()
{
    ...
    if ($type == "coc") {
        ...
        if ($da_tra_coc >= $tien_coc_40) {
            echo "<script>alert('Khách đã đóng đủ tiền cọc 40%.');window.location='?action=qlbooking_detail&id=$id';</script>";
            exit;
        }

        $so_tien_can_dong = $tien_coc_40 - $da_tra_coc;

        // ❌ Nếu nhập lớn hơn số tiền cọc còn lại → báo lỗi
        if ($so_tien > $so_tien_can_dong) {
            echo "<script>alert('Số tiền vượt quá số tiền cọc còn lại! Bạn chỉ được nhập tối đa " . number_format($so_tien_can_dong) . " VNĐ');history.back();</script>";
            exit;
        }
        ...
        echo "<script>alert('Thanh toán cọc thành công! Còn lại: " . number_format($con_lai) . " VNĐ');window.location='?action=qlbooking_detail&id=$id';</script>";
        exit;
    }
    ...
    if ($type == "full") {
        ...
        if ($da_tra_tong >= $gia_tour) {
            echo "<script>alert('Khách đã thanh toán đủ toàn bộ tour.');window.location='?action=qlbooking_detail&id=$id';</script>";
            exit;
        }
        ...
        if ($so_tien > $so_tien_can_dong) {
            echo "<script>alert('Số tiền vượt quá tổng giá tour! Bạn chỉ được nhập tối đa " . number_format($so_tien_can_dong) . " VNĐ');history.back();</script>";
            exit;
        }
        ...
        echo "<script>alert('Thanh toán FULL thành công! Khách còn phải đóng: " . number_format($con_lai) . " VNĐ');window.location='?action=qlbooking_detail&id=$id';</script>";
        exit;
    }
}
